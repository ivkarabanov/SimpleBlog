...
namespace JustBlog.Controllers
{
    public class BlogController : Controller
    {
        ...
        public ViewResult Posts(int p = 1)
        {
            ...
            ViewBag.Title = "Последние статьи";
        ...
        public ViewResult Category(string category, int p = 1)
        {
            ...
            ViewBag.Title = String.Format(@"Последние статьи в категории "" {0} "" ", viewModel.Category.Name);
            return View("List", viewModel);
        }
        ...
        public ViewResult Tag(string tag, int p = 1)
        {
            ...
            if (viewModel.Tag == null)
                throw new HttpException(404, "Тэг не найден");

            ViewBag.Title = String.Format(@"Последние статьи по тэгу ""{0}""",
                viewModel.Tag.Name);
            return View("List", viewModel);
        }

        public ViewResult Search(string search, int p = 1)
        {
            ViewBag.Title = String.Format(@"Список найденных статей по тексту запроса ""{0}""", search);

            var viewModel = new ListViewModel(_blogRepository, search, "Search", p);
            return View("List", viewModel);
        }
        ...
        {
            throw new Exception("У данного действия нет реализации!");
        }
